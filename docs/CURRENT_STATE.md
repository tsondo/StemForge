...
- **MIDI Out (MIDI tab)** — Web MIDI output to hardware synthesizers, spec'd in `docs/WEB_MIDI_OUT_SPEC.md` (rev. 3). Each card's routing dropdown selects **SoftSynth** (the FluidSynth preview) or a hardware port, and the card's single transport button drives whichever is chosen, so the two can never sound simultaneously; the waveform tracks and seeks hardware playback. Worker-driven lookahead scheduler (hidden tabs do not gap), two-phase stop, global Panic, optional Program Change with drum-stem suppression, per-stem port memory in localStorage. Merged MIDI is a full card of its own. Zero new dependencies — Web MIDI is a browser API, so no licensing surface. Chrome/Edge and a secure context required; validated on hardware (Korg modwave + wavestate)
...
- **ROCm variant** — `pyproject.toml.ROCM` (AMD GPUs, rocm7.1 torch index, same pins); resolution-verified only, awaiting community hardware validation (issue #11). `--gpu` also sets `HIP_VISIBLE_DEVICES`. AceStep needs no separate ROCm work — it is spawned with `sys.executable` so it inherits StemForge's venv, and the ROCM pyproject overrides its cu128 pins. The Wrangler submodule run *standalone* is still CUDA-only (own cu128 indexes + a prebuilt CUDA flash-attn wheel). CPU-bound under ROCm: UVR enhance (onnxruntime) and faster-whisper (ctranslate2)
...
- Automated testing coverage is thin but no longer absent: CI runs a pytest subset (compose backend, GPU scheduler, vocal MIDI, VP mode, drum MIDI routing, Web MIDI event flattening, MIDI render) on every PR; older script-style files in tests/ are not pytest-compatible and are excluded. The test job names each file explicitly, so a new suite is skipped until it is added to `.github/workflows/ci.yml`.
