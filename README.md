...
- **MIDI Out** — stream extracted MIDI straight to hardware synths via Web MIDI; each stem card routes to the built-in soft synth or any hardware port from one transport button, with a waveform that tracks and seeks hardware playback, optional Program Change, and a Panic button (Chrome/Edge required)
...
### MIDI
Extract MIDI from separated stems. BasicPitch handles polyphonic instrument stems; faster-whisper + PYIN pitch tracking handles vocal melodies. Per-stem FluidSynth preview lets you audition MIDI renderings directly in the browser via wavesurfer.js. **MIDI Out** streams any stem to hardware synthesizers over the Web MIDI API. Each card's MIDI Out dropdown picks its destination — SoftSynth (the FluidSynth preview) or any hardware port — and the card's single transport button drives whichever is selected, so the two can never sound at once. The waveform tracks hardware playback and click-to-seek works against the synth. Also supports simultaneous playback across multiple ports, optional Program Change, and a global Panic button. Requires Chrome or Edge and a secure context — see [MIDI Out from another machine](#midi-out-from-another-machine) below for what that means and how to get one.

#### MIDI Out from another machine

Web MIDI only works in a **secure context**: the page's origin must be HTTPS or a loopback address. This is a browser guarantee — no setting on the page can change it. On the machine running StemForge, `http://localhost:<port>` is already a secure context and nothing further is needed. But StemForge binds to `0.0.0.0`, so from a second machine you'd naturally browse `http://<server-ip>:8765` — an insecure origin, where the browser reports the MIDI permission as blocked and won't offer to change it. (If Brave or Chrome shows MIDI stuck on "Block" for the site, this is why — it's the origin, not a permission setting.) Three ways to get a secure context, in order of preference:

- **Tailscale (recommended for multi-device):** on the server run `tailscale serve 8765`. Tailscale terminates HTTPS with a publicly trusted, auto-renewed certificate on a `*.ts.net` hostname — no browser warnings, no CA to distribute, and it works from phones and tablets with nothing installed on them beyond Tailscale itself.
- **SSH tunnel (no certificates at all):** from the client machine run `ssh -L 8765:localhost:8765 user@server`, then browse `http://localhost:8765` there. The origin genuinely is loopback, so it's a secure context. Fastest option for a one-off test from another desktop.
- **Built-in TLS:** generate a certificate with [mkcert](https://github.com/FiloSottile/mkcert), install the mkcert CA on each client machine, then run `uv run stemforge --ssl-certfile cert.pem --ssl-keyfile key.pem`. StemForge then serves HTTPS directly (the flags mirror uvicorn's; supplying only one of the two is a startup error, not a silent fallback to HTTP). Two caveats: installing a custom CA on a phone or tablet is painful, and a bare self-signed certificate without a trusted CA is **not** sufficient in Chrome or Brave — clicking through the interstitial warning does not produce a usable secure context.

StemForge accepts a certificate and key; it does not generate, manage, or renew them — that's what mkcert (or your own CA / reverse proxy) is for. On Hugging Face Spaces none of this applies: Spaces already serves HTTPS, so MIDI Out works there with no setup.
